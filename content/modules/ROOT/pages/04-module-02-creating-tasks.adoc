...
To share data between steps, and between Tasks, Tekton uses **Workspaces**. When a Task runs, it often needs access to files — source code, credentials, build artifacts, and so on. Workspaces are how Tasks request that storage without locking in the implementation details.

A Task declares that it needs a workspace. The TaskRun decides how to provide it:

* ** A ConfigMap or Secret (read-only)**: Read-only data injected as files into the Task.
* ** Existing PersistentVolumeClaim**: Durable storage, survives pod restarts. Recommended for build artifacts. (useful for sharing data between Tasks)
* **A new PersistentVolumeClaim** created from a **VolumeClaimTemplate**: Same as above.
* **emptyDir**: Ephemeral storage, wiped when the pod exits. Useful for temporary scratch space - An emptyDir that lives only for the duration of the TaskRun

This separation makes Tasks reusable — the same Task can work with a shared volume in one pipeline and a temporary directory in another.
...
Also check the new task in the OpenShift console, like you did in Module 1 - Exercise 2. Check in **Pipelines** then **Tasks**
...
----
+
NOTE: OpenShift uses dynamic storage provisioning. The PVC moves from `Pending` to `Bound` automatically once storage is allocated/used.
...
In the OpenShift Console, confirm both TaskRuns succeeded using the same Task.

. View the TaskRuns list in the OpenShift console under **Pipelines**, then **Tasks**.
.. Both TaskRuns reference the same underlying Task > build-coolstuff-app.
...
[NOTE]
====
Note that the two different TaskRuns ran in different pods
====
