...
// TODO: Talk about the value of gitops - using console and manual yaml files in this ws but really good to store templates in git and use gitops for continuity and security
...
. We will manually create a named PipelineRun and execute it, to make it easier to follow up in these excersises.
...
. Click `coolstuff-pipeline-run-02` to open the pipeline graph. Each node shows its status icon and duration. Hover over a node(e.g. _build_) to see its TaskRun name(_validate, test, package_) and timing details.
...
. View the console failure view. In the OpenShift console, navigate to **Pipelines**, then **PipelineRuns** and then click `coolstuff-full-pipelinerun-01`, then `coolstuff-full-run-01`. The pipeline graph shows `build` in green and `deploy` in red.
