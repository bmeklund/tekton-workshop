...
* **PipelineRun** - A specific single execution of a Pipeline. A PipelineRun tracks the status of every Task in the Pipeline and stores the overall result. You can view the status of your CI/CD workflow, including the specifics of each task execution with pipelineRuns. A PipelineRun is the result of running an instance of Pipeline, which includes a number of TaskRun

image::tekton-architecture.png[Tekton architecture diagram showing how the Tekton controller watches for TaskRun and PipelineRun resources and schedules pods, 600]
...
You should already have logged in to the OpenShift Web Console, as well as have opened the OpenShift Web Terminal.

If you have done that, skip down to <<openshift-console>>. Otherwise follow the steps here:
...
[role='params-link']
image::developer-console-button.png[OpenShift console button to open the web console,300]
+
...
[window=_blank, align="center"]
[role='params-link']
image::developer-webterminal-button.png[OpenShift console button to open the web terminal,300]
+
...
+
* If it's the first time you start the Terminal you will need to initialize it. In the *Initialize Terminal* view enter the following:
...
The Red Hat OpenShift Container Platform web console is a graphical user interface accessible via a web browser that allows developers and administrators to visualize, browse, and manage project contents, cluster inventory, and resources.

The Web Console is governed by RBAC, meaning you can only view and interact with resources for which you have the appropriate permissions.

. When first logged in you will see an overview. The overview lists details on the current OpenShift project you are in. It shows details on your workloads in it. At the moment it is empty but this we will change during the course of this workshop.
...
Confirm the OpenShift Pipelines operator is installed and running:
...
We will start with using the Web Terminal, so switch to the terminal window.
