...
--
image::pipeline-icon.png[Pipelines icon,link=self,window=blank,width=100]

A Pipeline is a collection of Tasks that are executed in a defined order and can share inputs. Tekton organizes these tasks into a directed acyclic graph (DAG), ensuring they run in the correct sequence based on their dependencies. In practice, this means Tekton creates and manages a set of Kubernetes pods, making sure each one runs to completion successfully.

Tekton gives developers fine-grained control over execution. For example, it supports fan-in and fan-out patterns between tasks, allows automatic retries for unstable or flaky steps, and enables conditional execution so that tasks only run when specific criteria are met.

Both Tasks and Pipelines are defined as custom resources within a Kubernetes cluster, making them fully declarative and Kubernetes-native.

Three concepts are key to understanding how Pipelines work:
...
You'll create a Pipeline that chains two Tasks: the `build-coolstuff-app` Task from Module 2 and a new `summarize-build` Task that reports the build result. This gives you a complete build and report workflow in a single Pipeline.
...
* `tkn pipeline describe` shows two tasks with correct `runAfter` ordering
...
Open the OpenShift console and view the pipeline in the Pipelines section. Fold out **Pipelines** then click **Pipelines** and in the center view click the `coolstuff-build-pipeline`
...
.. In the OpenShift console, fold out the **Pipelines** section, then click **Pipelines** again and then select the **PipelineRuns** tab before clicking on the `coolstuff-pipeline-run-01`. Here you can see the visual graph of the executed PipelineRun as well as the overall status: `Succeeded`.
...
. You will now see your initialized repository with a basic readme file looking like the below picture. If you click **Code** you can note the repository clone URL displayed.
+
...
image::openshift-pipelines-logo.png[OpenShift Pipelines,link=self,window=blank,width=100]
...
NOTE: Doing it manually in the console is a good way to learn about OpenShift Pipelines and it's capabilities. It's not something you do in a production or even test environment though. You can also do this manually using the CLI as before, or more commonly done - using GitOps and ArgoCD.  
...
| URL | \https://gitea-workshop-gitea.%APPS_HOSTNAME_SUFFIX%/{OPENSHIFT_USERNAME}/coolstuff-app.git
| output | shared-data
|===
+
. Click on *Save* to save the configuration. Verify that the red exclamation mark is gone and the git-clone task is looking good.
...
.. But make sure to set `VolumeClaimTemplate` as the base for the *shared-data* workspace.
+
...
+
. When all the tasks in the pipeline have executed without issues the overview will be updated showing the status of the PipelineRun which should be: `Succeeded`.
. If you have jumped out of the view you can find it again in the console under **Pipelines** → PipelineRuns → `coolstuff-pipeline-_random-id_` 
...
----
Expected output (non sequential order)
+
----
...
* Created a `coolstuff-build-pipeline` Pipeline chaining two Tasks with `runAfter` ordering
* Ran the Pipeline using a PipelineRun and monitored it through the visual pipeline graph
* Added a `git-clone` Task that pulls real source code from the Gitea server
* Extended the Pipeline to three stages: `git-clone`, `build`, and `summarize`
