...
. View the 3-stage pipeline graph in the console under **Pipelines** → PiplineRuns → `coolstuff-pipeline-_random-id_` Click the pipelineRun and verify its all green and marked as Succeeded.
...
NOTE: You will have to check what the random id appended on your pipelinerun-instance is and adjust the below command with your id instead. You can see the specific id appended on your pipelineRun in the OpenShift console.
...
Confirm the updated Pipeline has 3 stages(again referencing your specific instance id):

[source,sh,role="copypaste"]
----
tkn pipeline describe coolstuff-build-pipeline-CHANGE_ME
----
...
* `coolstuff-build-pipeline-_random-id_` shows `Succeeded`
