...
<<TODO>> fix the pipeline-as-code-controller route....https://pipelines-as-code-controller-openshift-pipelines.apps.cluster-flqhr.flqhr.sandbox5312.opentlc.com
+
[source,sh,role="copypaste"]
----
...
* **Permissions**: select **repo: public_repo** and **admin:repo_hook: write:repo_hook**
...
. Click on the pencil on the newly created webhook and verify at the bottom of the screen that all looks good(green checkmark on the webhook)

image::gitea-webhook-verify.png[Webhook verification in Gitea,link=self,window=blank,width=700]

. Click **Test delivery** to confirm Gitea can reach the PAC controller. A `202` response confirms the connection is working.
